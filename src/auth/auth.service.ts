...
export class AuthService {
  ...
  async signup(signupDto: SignupDto): Promise<{ message: string }> {
    try {
      ...
      const huhu = await this.cacheManager.get(`verify:${signupDto.email}`);
      const test = await this.cacheManager.set('test_key', '12345', 600);
      ...
      console.log('Saved code to Redis:', huhu);
